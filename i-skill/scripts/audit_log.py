...
class AuditLog:
    def __init__(self, user_data_path: str = "./user_data", config_path: str = None):
        ...
        self.security_log_file = self.user_data_path / "security_log.json"
        self.metrics_file = self.user_data_path / "audit_metrics.json"
        
        self._initialize_audit_log()
        self._initialize_security_log()
    ...
    def _initialize_security_log(self):
    ...
    def _save_security_log(self, log: List[Dict]):
        with open(self.security_log_file, 'w', encoding='utf-8') as f:
            json.dump(log, f, indent=2)
    ...
    def log(self, action: AuditAction, skill_name: Optional[str] = None, 
            details: Optional[Dict] = None, level: AuditLevel = AuditLevel.INFO,
        ...
        try:
            if level in [AuditLevel.ERROR, AuditLevel.CRITICAL]:
                self._log_to_security_log(log_entry)
            else:
    ...
    def _should_log(self, action: AuditAction) -> bool:
        if action == AuditAction.READ and not self.config["audit"]["log_all_access"]:
            return False
        if action == AuditAction.WRITE and not self.config["audit"]["log_all_writes"]:
            return False
        if action in [AuditAction.CONSENT_GRANTED, AuditAction.CONSENT_DENIED, AuditAction.CONSENT_REVOKED] and not self.config["audit"]["log_all_consents"]:
            return False
        if action == AuditAction.VALIDATION_FAILED and not self.config["audit"]["log_validation_failures"]:
            return False
        if action == AuditAction.SANITIZATION_APPLIED and not self.config["audit"]["log_sanitization"]:
            return False
    ...
    def _log_to_security_log(self, log_entry: Dict):
        log = self._load_security_log()
        log.append(log_entry)

        if self.config["audit"]["log_rotation"] and len(log) > self.config["audit"]["max_log_size"]:
            log = log[-self.config["audit"]["max_log_size"]:]  # 修复日志轮转

        self._save_security_log(log)  # 保存截断后的日志
    ...
    def get_security_log(self, skill_name: Optional[str] = None,
                        level: Optional[AuditLevel] = None,
                        limit: int = 100,
                        offset: int = 0) -> List[Dict]:
        log = self._load_security_log()
    ...
    def get_anomaly_report(self) -> Dict:
        ...
        return {
            ...
            "recent_security_events": len(self.get_security_events(hours=24)),
